...
`/tw/events/` — `scripts/fetch-tw-events.mjs` + `.github/workflows/update-tw-events.yml`(하루 3회). 첫 수집 40건.

한국어판 `fetch-events.mjs`와 같은 사고방식이다 — 뉴스형 수집원에 **행사 조건을 겹쳐** 거른다. 수집원은 두 가지:
- **中央社 검색** — 행사 지향 검색어(`智慧醫療 研討會`·`AI醫療 論壇`·`醫療科技展`·`智慧醫療 工作坊`·`AI 醫療 講座`). 뉴스 수집기와 같은 JSON-LD ItemList 방식.
...
⚠️ **이 목록은 '행사 안내'가 아니라 '행사를 다룬 보도'다.** 날짜는 보도 게재일이지 개최일·신청 마감일이 아니고, 지난 행사도 남는다. 화면 상단의 `.tw-caution` 안내가 그것을 밝힌다 — **지우지 말 것.** 방문자가 신청 가능한 목록으로 오해하면 안 된다.

...
- **Phase C(팁·강의영상·뉴스 자동수집)·Phase D(블로그)**: 계획서에 설계는 돼 있으나 미착수.
- **지원사업(補助金 대응) 자동화**: 일본 jGrants 같은 공개 API형 소스를 러시아에서 찾지 못했다. 후속 조사 필요(오너 확인 필요 항목, §8).
- **뉴스 자동 수집**: Medvestnik(medvestnik.ru)·Vademecum(vademec.ru)을 후보로 조사했으나 실제 스크래핑 가능 여부(HTML 구조)는 미확인 — Phase C 착수 시 WebFetch로 먼저 확인할 것.

...
<!--
  인도네시아어판 RSS 피드 표시용 스타일시트 (2026-09-06 신설).
  한국어판 /rss-styles.xsl의 복제이고 문구만 인도네시아어로 바꿨다.
  (이 저장소는 언어판끼리 파일을 공유하지 않는 관례를 따른다 — 한 언어를
   고치다 다른 언어가 휘말리는 사고를 막기 위해서다.)
  글꼴은 한국어판과 같다 — 라틴 문자라 대체가 필요 없다.
-->
  ...
  <xsl:template match="/rss/channel">
    <html lang="id">
      <head>
        ...
        <title><xsl:value-of select="title" /> · Umpan RSS</title>
        <style>
          :root {
            --primary: #0b5394;
            --text: #222222;
      ...
      <body>
        <div class="wrap">
          <span class="badge">Umpan RSS</span>
          <h1><xsl:value-of select="title" /></h1>
          <p class="desc"><xsl:value-of select="description" /></p>

          <div class="note">
            <strong>Halaman ini adalah umpan RSS.</strong>
            Untuk menerima tulisan baru secara otomatis, salin alamat di bilah alamat
            ke aplikasi pembaca RSS (misalnya Feedly atau Inoreader). Daftar di bawah
            adalah tulisan terbaru. Untuk membacanya di situs, klik judulnya.
          </div>
          ...
          <a class="home" href="{link}">← Ke beranda Hospital AI Lab</a>
        </div>
      </body>
    </html>
  </xsl:template>

...
<!--
  러시아어판 RSS 피드 표시용 스타일시트 (2026-09-06 신설).
  한국어판 /rss-styles.xsl의 복제이고 문구만 러시아어로 바꿨다.
  (이 저장소는 언어판끼리 파일을 공유하지 않는 관례를 따른다 — 한 언어를
   고치다 다른 언어가 휘말리는 사고를 막기 위해서다.)
  글꼴은 한국어판과 같다 — Pretendard가 키릴을 지원하므로 대체가 필요 없다.
-->
  ...
  <xsl:template match="/rss/channel">
    <html lang="ru">
      <head>
        ...
        <title><xsl:value-of select="title" /> · RSS-лента</title>
        <style>
          :root {
            --primary: #0b5394;
            --text: #222222;
      ...
      <body>
        <div class="wrap">
          <span class="badge">RSS-лента</span>
          <h1><xsl:value-of select="title" /></h1>
          <p class="desc"><xsl:value-of select="description" /></p>

          <div class="note">
            <strong>Эта страница — RSS-лента.</strong>
            Чтобы автоматически получать новые материалы, вставьте адрес из адресной
            строки в программу для чтения лент (например, Feedly или Inoreader).
            Ниже показаны последние материалы. Чтобы читать на сайте, нажмите на заголовок.
          </div>
          ...
          <a class="home" href="{link}">← На главную страницу</a>
        </div>
      </body>
    </html>
  </xsl:template>

...
1. [카카오 디벨로퍼스](https://developers.kakao.com/console/app) → **애플리케이션 추가**로 앱 생성. 이 화면의 **회사명**은 실제 사업자명이 아니어도 됩니다 — 개인 개발자는 이름이나 서비스명(예: "병원 AI 연구소")을 자유롭게 적으면 되고, 나중에 얼마든지 바꿀 수 있습니다. **사업자등록번호는 이 단계에서 필요 없습니다.**

2. **[중요] 개인 개발자 비즈 앱 전환** — 카카오는 일반 앱에서 이메일을 아예 동의항목으로 주지 않습니다(선택 동의로도 안 뜸). 저희 사이트는 회원 이메일이 필요하므로 이 전환이 꼭 필요합니다. 사업자등록번호 없이도 신청 가능합니다.
   - 왼쪽 사이드바 **앱 설정 → 앱 → 일반 → 비즈니스 정보** → **"개인 개발자 비즈 앱 전환"** 버튼 클릭 후 신청
   - 심사 기준: 실제 운영 서비스와 등록한 정보가 일치하는지, 운영정책 준수 여부, 서비스 신뢰성 — 사이트 주소(hospital-ai-lab.com)와 회원가입 화면을 그대로 보여주면 됩니다
   - 심사 소요 기간은 카카오 쪽 사정이라 저도 정확히 알 수 없습니다. 승인 전까지는 아래 4단계에서 "카카오계정(이메일)" 항목이 목록에 보이지 않을 수 있습니다 — 정상이니 승인을 기다리면 됩니다

3. 왼쪽 사이드바 **앱 설정 → 앱 → 플랫폼 키 / 어드민 키** 탭에서 **REST API 키** 복사 (이것이 client_id). 같은 탭 안에 있는 항목:
...
4. 왼쪽 사이드바 **제품 설정 → 카카오 로그인 → 사용 설정**에서 상태를 **ON**으로 켜기
5. **제품 설정 → 카카오 로그인 → 동의항목**에서 닉네임(profile_nickname)과 카카오계정(이메일, account_email)을 "필수 동의"로 설정 (2단계 승인 후에만 이메일 항목이 보입니다)
6. Supabase 대시보드 **Authentication → Providers → Kakao**에 3단계에서 복사한 REST API 키(client_id)와 Client Secret 코드(client_secret)를 입력하고 저장, 활성화
